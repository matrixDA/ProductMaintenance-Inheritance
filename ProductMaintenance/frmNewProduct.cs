namespace ProductMaintenance
{
    public partial class frmNewProduct : Form
    {
        ...
        {
            //Add code when the check box is changed
            if (rbBook.Checked)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (IsValidData())
            {
                //Add code here for when a Book or Software is Added
              //  product = new Product(txtCode.Text, txtDescription.Text, Convert.ToDecimal(txtPrice.Text));
                if (rbBook.Checked)
                {
                    product = new Book(txtCode.Text, txtDescription.Text, Convert.ToDecimal(txtPrice.Text), txtAuthorOrVersion.Text);
                }
                else
                {
                    product = new Software(txtCode.Text, txtDescription.Text, Convert.ToDecimal(txtPrice.Text), txtAuthorOrVersion.Text);
                }
                this.Close();
            }
        }
